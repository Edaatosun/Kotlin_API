...
import com.eda.bookapp2.databinding.ActivityFavouritePageBinding
...
class favouritePage : AppCompatActivity(),BookRecycleViewAdapter.Listener {
    private lateinit var binding: ActivityFavouritePageBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityFavouritePageBinding.inflate(layoutInflater)
    ...
    private fun loadData(userId: String) {
        ...
        val call = service.showFavorites(userId)

        call.enqueue(object : Callback<List<String>> {
            override fun onFailure(call: Call<List<String>>, t: Throwable) {
                t.printStackTrace()
                Toast.makeText(this@favouritePage, "Veriler yüklenemedi: ${t.message}", Toast.LENGTH_SHORT).show()
            }
            ...
            override fun onResponse(call: Call<List<String>>, response: Response<List<String>>) {
                if (response.isSuccessful) {
                    response.body()?.let { favoriteBookIds ->
                        ...
                        Toast.makeText(this@favouritePage, "Favoriler alınamadı.", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    Toast.makeText(this@favouritePage, "Yanıt hatası", Toast.LENGTH_SHORT).show()
                }
            }
        })
    }
        ...
        for (call in bookDetailsCalls) {
            call.enqueue(object : Callback<Book> {
                override fun onFailure(call: Call<Book>, t: Throwable) {
                    t.printStackTrace()
                    Toast.makeText(this@favouritePage, "Kitap verisi yüklenemedi.", Toast.LENGTH_SHORT).show()
                }

                override fun onResponse(call: Call<Book>, response: Response<Book>) {
                    if (response.isSuccessful) {
                        response.body()?.let { book ->
                            // Kitap modelini ekliyoruz
                            bookModels.add(book)
                            // RecyclerView güncelleme işlemi (eğer RecyclerView kullanıyorsanız)
                            updateRecyclerView(bookModels)
                        } ?: run {
                            Toast.makeText(this@favouritePage, "Kitap verisi alınamadı.", Toast.LENGTH_SHORT).show()
                        }
                    } else {
                        Toast.makeText(this@favouritePage, "Kitap verisi alınamadı.", Toast.LENGTH_SHORT).show()
                    }
    ...
    private fun updateRecyclerView(books: ArrayList<Book>) {
        recyclerViewAdapter = BookRecycleViewAdapter(books, this@favouritePage)
